(() => {
  ...
  const GYM_QUERY={
    bench_press:'barbell bench press',incline_db_press:'dumbbell incline bench press',incline_machine_press:'lever incline chest press',pec_deck:'lever seated fly',cable_fly:'cable middle fly',triceps_pushdown:'cable pushdown',overhead_triceps:'cable overhead triceps extension',lateral_raise:'dumbbell lateral raise',pullup:'pull-up',lat_pulldown:'cable lat pulldown',machine_row:'lever seated row',cable_row:'cable seated row',reverse_pec_deck:'lever reverse fly',barbell_curl:'barbell curl',hammer_curl:'dumbbell hammer curl',smith_shoulder_press:'smith shoulder press',reverse_fly:'dumbbell reverse fly',hack_squat:'sled hack squat',rdl:'barbell romanian deadlift',leg_curl:'lever lying leg curl',calf_raise:'standing calf raise',hanging_leg_raise:'hanging leg raise',cable_crunch:'cable kneeling crunch',incline_press_any:'dumbbell incline bench press',pulldown_any:'cable lat pulldown',crunch_combo:'crunch',machine_press:'lever chest press',bulgarian_split_squat:'dumbbell bulgarian split squat',biceps_curl:'dumbbell biceps curl',triceps_pressdown:'cable pushdown',front_raise:'barbell front raise',barbell_shoulder_press:'barbell standing military press',shrug:'dumbbell shrug',flat_db_press:'dumbbell bench press',decline_press:'barbell decline bench press',chest_dip:'chest dip',deadlift:'barbell deadlift',barbell_row:'barbell bent over row',one_arm_db_row:'dumbbell one arm bent-over row',tbar_row:'t-bar row',face_pull:'cable standing face pull',arnold_press:'dumbbell arnold press',upright_row:'cable upright row',back_squat:'barbell full squat',leg_press:'sled 45 leg press',leg_extension:'lever leg extension',hip_thrust:'barbell hip thrust',cable_kickback:'cable glute kickback',preacher_curl:'barbell preacher curl',cable_curl:'cable curl',skull_crusher:'barbell lying triceps extension',close_grip_bench:'barbell close-grip bench press',plank:'front plank',ab_wheel:'wheel rollout',wrist_curl:'barbell wrist curl',treadmill_run:'run',incline_walk:'walking on treadmill',elliptical:'elliptical machine walk',stationary_bike:'stationary bike run',rowing_machine:'rowing machine',stair_climber:'walking on stepmill',jump_rope:'rope skipping',swimming:'swimming'
  };
  const VIDEO_MAP={};
  ...
  function showAddExercise(){
    openModal('添加动作',`<div class="search"><input id="modal-ex-search" placeholder="搜索动作"></div><div id="modal-ex-list" class="list">${EXERCISES.map(e=>`<button class="list-item" style="width:100%;text-align:left" data-pick-ex="${e.id}"><div class="exercise-thumb">${exerciseVisual(e)}</div><div class="grow"><strong>${esc(e.name)}</strong><small>${e.group} · ${e.equipment}</small></div><span class="pill blue">添加</span></button>`).join('')}</div>`);
    const bind=()=>document.querySelectorAll('[data-pick-ex]').forEach(b=>b.onclick=()=>{const e=exercise(b.dataset.pickEx);state.activeWorkout.exercises.push({exerciseId:e.id,target:null,sets:[{weight:'',reps:'',minutes:'',warmup:false,done:false,drops:[]}]});pendingFreeWorkout=false;saveState();closeModal();renderActiveWorkout();}); bind();
    document.getElementById('modal-ex-search').oninput=e=>{const q=e.target.value.trim();document.getElementById('modal-ex-list').innerHTML=EXERCISES.filter(x=>x.name.includes(q)).map(x=>`<button class="list-item" style="width:100%;text-align:left" data-pick-ex="${x.id}"><div class="exercise-thumb">${exerciseVisual(x)}</div><div class="grow"><strong>${esc(x.name)}</strong><small>${x.group} · ${x.equipment}</small></div><span class="pill blue">添加</span></button>`).join('');hydrateGymVisuals(document.getElementById('modal-ex-list'));bind();};
  }
  ...
  function showPlanExercisePicker(p,mode,index=null){openModal(mode==='add'?'添加动作':'替换动作',`<div class="search"><input id="plan-pick-search" placeholder="搜索动作"></div><div id="plan-pick-list" class="list"></div>`);const draw=(q='')=>{document.getElementById('plan-pick-list').innerHTML=EXERCISES.filter(e=>e.name.includes(q)).map(e=>`<button class="list-item" data-plan-pick="${e.id}"><div class="exercise-thumb">${exerciseVisual(e)}</div><div class="grow"><strong>${esc(e.name)}</strong><small>${e.group} · ${e.equipment}</small></div></button>`).join('');hydrateGymVisuals(document.getElementById('plan-pick-list'));document.querySelectorAll('[data-plan-pick]').forEach(b=>b.onclick=()=>{if(mode==='add')p.exercises.push([b.dataset.planPick,1,8,12]);else{const old=p.exercises[index];p.exercises[index]=[b.dataset.planPick,old[1],old[2],old[3]];}saveState();showPlan(p.id);});};draw();document.getElementById('plan-pick-search').oninput=e=>draw(e.target.value.trim());}

  function exerciseVisual(ex){return `<img class="gym-gif" data-gym-id="${ex.id}" alt="${esc(ex.name)}" draggable="false">`;}
  async function loadGymIndex(){if(!gymIndexPromise)gymIndexPromise=fetch(GYM_DATA_URL,{cache:'force-cache'}).then(r=>r.ok?r.json():[]).catch(()=>[]);return gymIndexPromise;}
  function norm(s=''){return String(s).toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();}
  async function gymRecord(ex){if(gymCache[ex.id])return gymCache[ex.id];const list=await loadGymIndex(),q=norm(GYM_QUERY[ex.id]||'');if(!q)return null;let rec=list.find(x=>norm(x.name)===q)||list.find(x=>norm(x.name).includes(q))||list.find(x=>q.includes(norm(x.name)));gymCache[ex.id]=rec||null;return rec||null;}
  async function hydrateGymVisuals(root=document){const imgs=[...root.querySelectorAll('img.gym-gif:not([data-hydrated])')];for(const img of imgs){img.dataset.hydrated='1';const ex=exercise(img.dataset.gymId);const rec=await gymRecord(ex);if(!rec){img.classList.add('no-media');img.alt=ex.name;continue;}let url=rec.gif_url||rec.video||rec.gif||'';if(url&&!/^https?:/i.test(url))url=`${GYM_BASE}/${String(url).replace(/^\.\//,'')}`;if(!url&&rec.media_id)url=`${GYM_BASE}/videos/${rec.media_id}.gif`;img.src=url;img.onerror=()=>{img.classList.add('no-media');};}}
  ...
  function muscleMap(ex){const pri=canonicalMuscles(ex.primary||ex.group),sec=canonicalMuscles(ex.secondary||'').filter(x=>!pri.includes(x));const all=new Set([...pri,...sec]);const c=n=>pri.includes(n)?'#ff2026':sec.includes(n)?'#ff9f1a':'#f7f7f7';const stroke='#252525';const label=(n,x1,y1,x2,y2,side='left')=>all.has(n)?`<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${c(n)}" stroke-width="1.5"/><circle cx="${x1}" cy="${y1}" r="2.5" fill="${c(n)}"/><text x="${side==='left'?x2-6:x2+6}" y="${y2+4}" text-anchor="${side==='left'?'end':'start'}" fill="${c(n)}" font-size="12" font-weight="700">${n}</text>`:'';return `<div class="heatmap-wrap"><svg viewBox="0 0 620 390" class="heatmap-svg" role="img" aria-label="${esc(ex.name)}训练肌群热力图"><rect width="620" height="390" rx="18" fill="#fff"/><g stroke="${stroke}" stroke-width="1.7" fill="#fff"><circle cx="205" cy="45" r="25"/><path d="M185 73 Q205 92 225 73 L240 125 229 212 219 340 191 340 181 212 170 125Z"/><path d="M170 105 L140 185 153 195 190 135"/><path d="M240 105 L270 185 257 195 220 135"/><circle cx="415" cy="45" r="25"/><path d="M395 73 Q415 92 435 73 L450 125 439 212 429 340 401 340 391 212 380 125Z"/><path d="M380 105 L350 185 363 195 400 135"/><path d="M450 105 L480 185 467 195 430 135"/></g><g stroke="#fff" stroke-width="1"> <path d="M181 100 Q205 82 229 100 L225 132 Q205 145 185 132Z" fill="${c('胸大肌')}"/><path d="M168 101 Q180 85 190 92 L188 126 174 134Z" fill="${c('三角肌前束')}"/><path d="M230 92 Q242 85 252 101 L246 134 232 126Z" fill="${c('三角肌前束')}"/><path d="M145 135 L159 133 153 178 141 174Z" fill="${c('肱二头肌')}"/><path d="M251 133 L265 135 269 174 257 178Z" fill="${c('肱二头肌')}"/><path d="M150 176 L159 178 153 205 144 201Z" fill="${c('前臂肌群')}"/><path d="M251 178 L260 176 266 201 257 205Z" fill="${c('前臂肌群')}"/><rect x="190" y="137" width="30" height="60" rx="12" fill="${c('腹直肌')}"/><path d="M184 200 L204 203 200 278 180 276Z" fill="${c('股四头肌')}"/><path d="M206 203 L226 200 230 276 210 278Z" fill="${c('股四头肌')}"/><path d="M183 280 L199 281 195 334 181 334Z" fill="${c('腓肠肌')}"/><path d="M211 281 L227 280 229 334 215 334Z" fill="${c('腓肠肌')}"/><path d="M392 103 Q415 87 438 103 L435 151 395 151Z" fill="${c('斜方肌')}"/><path d="M388 126 L401 130 398 191 382 179Z" fill="${c('背阔肌')}"/><path d="M442 130 L455 126 448 179 432 191Z" fill="${c('背阔肌')}"/><path d="M375 133 L389 131 385 178 373 174Z" fill="${c('肱三头肌')}"/><path d="M441 131 L455 133 457 174 445 178Z" fill="${c('肱三头肌')}"/><path d="M391 192 Q415 176 439 192 L438 225 Q415 239 392 225Z" fill="${c('臀大肌')}"/><path d="M392 228 L411 228 407 282 390 280Z" fill="${c('腘绳肌群')}"/><path d="M419 228 L438 228 440 280 423 282Z" fill="${c('腘绳肌群')}"/><path d="M391 282 L407 283 405 334 391 334Z" fill="${c('腓肠肌')}"/><path d="M423 283 L439 282 439 334 425 334Z" fill="${c('腓肠肌')}"/></g>${label('胸大肌',205,110,95,105,'left')}${label('三角肌前束',175,106,95,75,'left')}${label('肱二头肌',151,150,90,145,'left')}${label('前臂肌群',150,190,90,190,'left')}${label('腹直肌',205,165,95,225,'left')}${label('股四头肌',188,235,90,270,'left')}${label('腓肠肌',190,305,90,320,'left')}${label('斜方肌',415,112,525,90,'right')}${label('背阔肌',390,155,525,145,'right')}${label('肱三头肌',380,152,525,180,'right')}${label('臀大肌',415,205,525,225,'right')}${label('腘绳肌群',432,250,525,265,'right')}${label('腓肠肌',430,305,525,320,'right')}</svg><div class="heatmap-legend"><span><i class="red"></i>主要训练肌群</span><span><i class="orange"></i>辅助训练肌群</span><span><i class="gray"></i>非主要肌群</span></div></div>`;}
  ...
  function showTutorial(id){
    ...
    openModal(ex.name,`<div class="tutorial-section"><div class="tutorial-label">Gym Visual 循环 GIF</div><div class="demo-stage gym-stage">${exerciseVisual(ex)}<small>${esc(ex.name)}</small></div></div><div class="tutorial-section"><div class="tutorial-label">中文短视频</div>${videoHTML(ex)}</div><div class="tutorial-section"><div class="tutorial-label">动作要点</div><ol>${(ex.tips||[]).map(x=>`<li>${esc(x)}</li>`).join('')}</ol></div><div class="tutorial-section"><div class="tutorial-label">常见错误</div><ol class="mistakes">${(ex.mistakes||[]).map(x=>`<li>${esc(x)}</li>`).join('')}</ol></div><div class="tutorial-section"><div class="tutorial-label">训练肌群</div>${muscleMap(ex)}</div>`);
    requestAnimationFrame(()=>bindLocalTutorialVideos(modalBody));
  }
